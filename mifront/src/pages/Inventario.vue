  ...
  <div>
    <q-header bordered class="text-black bg-white">
      <q-toolbar>
        ...
        <q-btn  outline :label="$q.screen.lt.md?'Catego':'Crear categoría'" no-caps @click="clickCreateCategoria"/>
        <q-btn-dropdown
          class="q-ml-xs"
          split1
          color="warning"
    ...
    <q-page class="q-pa-xs">
      <div class="row">
          ...
          <q-card bordered flat>
            <q-item >
              ...
              <q-item-section>
                ...
                <q-item-label class="text-h5">{{productos.length}}</q-item-label>
              </q-item-section>
            </q-item>
          </q-card>
        </div>
        ...
        <div class="col-12">
          ...
          <!--&lt;!&ndash;                {{productos.length}} productos&ndash;&gt;-->
          <!--&lt;!&ndash;                {{optionsCategoriasVer.label}}&ndash;&gt;-->
        ...
        <q-card style="width: 450px; max-width: 80vw;">
          ...
          <q-card-section class="row items-center no-wrap">
            <q-form class="full-width">
              <div class="row">
                <div class="col-12">
                  <q-table :filter="filtercategoria" flat bordered hide-header hide-bottom :rows="categorias" :columns="columnsproducto">
                    <template v-slot:top>
...
export default {
  ...
  methods:{
    ...
    updateProducto(){
      ...
      this.$api.put("producto/"+this.producto.id,this.producto).then(res=>{
    ...
    createProducto(){
      this.producto.categoria_id=this.producto.categorias.id
      this.producto.negocio_id=this.store.negocio.id
      if (this.foto!=''){
        this.producto.foto=this.foto
      }
      if (this.producto.categoria_id==''){
    ...
    createCategoria(){
      this.$q.loading.show()
      this.categoria.negocio_id=this.store.negocio.id
      this.$api.post('categoria',this.categoria).then(res=>{
        this.$q.loading.hide()
        this.miscategorias()
    ...
    clickDetalleProducto(producto){
      ...
      let cat=this.categorias.find(c=>c.id===this.producto.categoria_id)
  ...
  computed:{
    costototal(){
      ...
      this.productos.forEach(p=>{
        if (p.cantidad>0){
          ct=ct+ parseFloat(p.costo) * parseFloat(p.cantidad)
        }
      })
      return ct.toFixed(2)
    }
